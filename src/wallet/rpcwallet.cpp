...
UniValue encryptwallet(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (!pwalletMain->EncryptWallet(strWalletPass)) {
        ...
        //shutdown
        StartShutdown();
        return "wallet encryption failed; Elosys server stopping, restart to restore unencrypted wallet.";
    } else {
...
UniValue z_sign_offline(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp==false)
    {
      ...
      if (sProject != "arrr")
      {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Only Elosys Chain (arrr) supported");
      }
