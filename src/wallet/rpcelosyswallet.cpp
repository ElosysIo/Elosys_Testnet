...
UniValue zs_listtransactions(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
  ...
  if (fHelp || params.size() > 5 || params.size() == 2)
      throw runtime_error(
        "zs_listtransactions\n"
        "\nReturns an array of decrypted Elosys transactions.\n"
        "\n"
        ...
        "      \"address\": \"address\",            (string)  Elosys address\n"
        "      \"scriptPubKey\": \"script\",        (string)  Script for the Elosys transparent address (transparent address type only)\n"
...
UniValue zs_gettransaction(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
  ...
  if (fHelp || params.size() != 1)
      throw runtime_error(
        "zs_gettransaction\n"
        "\nReturns a decrypted Elosys transaction.\n"
        "\n"
        "\nArguments:\n"
...
UniValue zs_listspentbyaddress(const UniValue& params, bool fHelp, const CPubKey& mypk) {
  ...
  if (fHelp || params.size() > 5 || params.size() == 3 || params.size() < 1)
      throw runtime_error(
        ...
        "\nReturns decrypted Elosys spent inputs for a single address.\n"
        "\n"
        "This function only returns information on addresses with full spending keys."
        "\n"
        "\nArguments:\n"
...
UniValue zs_listreceivedbyaddress(const UniValue& params, bool fHelp, const CPubKey& mypk) {
  ...
  if (fHelp || params.size() > 5 || params.size() == 3 || params.size() < 1)
      throw runtime_error(
        ...
        "\nReturns decrypted Elosys received outputs for a single address.\n"
        "\n"
        "This function only returns information on addresses with full spending keys."
        "\n"
        "\nArguments:\n"
...
UniValue zs_listsentbyaddress(const UniValue& params, bool fHelp, const CPubKey& mypk) {
  ...
  if (fHelp || params.size() > 5 || params.size() == 3 || params.size() < 1)
      throw runtime_error(
        ...
        "\nReturns decrypted Elosys outputs sent to a single address.\n"
        "\n"
...
{   //  category              name                          actor (function)              okSafeMode
    //  --------------------- ------------------------      -----------------------       ----------
    {   "elosys Exclusive",     "zs_listtransactions",       &zs_listtransactions,          true },
    {   "elosys Exclusive",     "zs_gettransaction",         &zs_gettransaction,            true },
    {   "elosys Exclusive",     "zs_listspentbyaddress",     &zs_listspentbyaddress,        true },
    {   "elosys Exclusive",     "zs_listreceivedbyaddress",  &zs_listreceivedbyaddress,     true },
    {   "elosys Exclusive",     "zs_listsentbyaddress",      &zs_listsentbyaddress,         true },
    {   "elosys Exclusive",     "getalldata",                &getalldata,                   true },
};

void RegisterElosysExclusiveRPCCommands(CRPCTable &tableRPC)
{
