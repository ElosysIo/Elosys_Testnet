...
const QString KOMODO_IPC_PREFIX("elosys:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the elosys: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(KOMODO_IPC_PREFIX, Qt::CaseInsensitive)) // elosys: URI
        {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click elosys: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start elosys: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling elosys: URIs and PaymentRequest mime types.
...
//         delete netManager;
//
//     // netManager is used to fetch paymentrequests given in elosys: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(KOMODO_IPC_PREFIX, Qt::CaseInsensitive)) // elosys: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseKomodoURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Elosys address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
