...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool allowZAddresses)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Elosys address (e.g. %1)").arg(QString("zs1u39a4y9g8tn...hnc2csmcl")));
#endif
...
{
    // return if URI is not valid or is no elosys: URI
    if(!uri.isValid() || uri.scheme() != QString("elosys"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!KomodoUnits::parse(KomodoUnits::ELO, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert elosys:// to elosys:
    //
    //    Cannot handle this later, because elosys:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("elosys://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "elosys:");
    }
...
{
    QString ret = QString("elosys:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(KomodoUnits::format(KomodoUnits::ELO, info.amount, false, KomodoUnits::separatorNever));
        paramCount++;
    }
