...
WalletFrame::WalletFrame(const PlatformStyle *_platformStyle, ElosysOceanGUI *_gui) :
    QFrame(_gui),
    gui(_gui),
...
bool WalletFrame::addWallet(const QString& name, WalletModel *walletModel)
{
    ...
    walletView->setElosysOceanGUI(gui);
    ...
    if (newInstall == true)
    {
        ...
        {
            // Default configuration files for the QT GUI & server (ELOSYS.conf)
    ...
    if (nMaxConnections==0)
    {
        ...
        printf("To disable all network connections, set the following in ELOSYS.conf\n");
        printf("  server=0\n");
        printf("  listen=0\n");
    }
    else
