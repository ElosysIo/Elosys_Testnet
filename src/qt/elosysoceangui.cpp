...
const std::string ElosysOceanGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ElosysOceanGUI::DEFAULT_WALLET = "~Default";

ElosysOceanGUI::ElosysOceanGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void ElosysOceanGUI::createActions()
{
    ...
    //sendCoinsAction->setStatusTip(tr("Send coins to a Elosys address"));
    ...
    //Note: elosysoceangui() gets run before the configuration file is scanned.
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Elosys transparent addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Elosys transparent addresses"));
    ...
    openAction->setStatusTip(tr("Open a elosys: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Elosys command-line options").arg(tr(PACKAGE_NAME)));
...
void ElosysOceanGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void ElosysOceanGUI::createToolBars()
{
    if(walletFrame)
    {
...
void ElosysOceanGUI::setColdStorageLayout( )
{
    QSettings settings;
    bool fEnableZSigning = settings.value("fEnableZSigning").toBool();
    if (fEnableZSigning==true)
    {
        ...
        //This can be accomplished by editing ELOSYS.conf and setting maxconnections=0.
...
void ElosysOceanGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ElosysOceanGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ElosysOceanGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ElosysOceanGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ElosysOceanGUI::setWalletActionsEnabled(bool enabled)
{
...
void ElosysOceanGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void ElosysOceanGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void ElosysOceanGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void ElosysOceanGUI::macosDockIconActivated()
{
    show();
    activateWindow();
}
...
void ElosysOceanGUI::optionsClicked()
{
...
void ElosysOceanGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void ElosysOceanGUI::showDebugWindow()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::showDebugWindowActivateConsole()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::showHelpMessageClicked()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
#ifdef ENABLE_WALLET
void ElosysOceanGUI::openClicked()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::resetUnlockTimer() {
    if (walletFrame) walletFrame->resetUnlockTimer();
}

void ElosysOceanGUI::gotoOverviewPage()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::gotoHistoryPage()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::gotoReceiveCoinsPage()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::gotoZSendCoinsPage(QString addr)
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::gotoZSignPage()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::gotoImportSK()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::gotoImportVK()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::showSeedPhrase()
{
    if (walletFrame) walletFrame->resetUnlockTimer();
...
void ElosysOceanGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Elosys network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void ElosysOceanGUI::setNumConnections(int count)
{
    (void)count;
    updateNetworkState();
}

void ElosysOceanGUI::setNetworkActive(bool networkActive)
{
    (void)networkActive;
    updateNetworkState();
}

void ElosysOceanGUI::updateHeadersSyncProgressLabel()
{
...
void ElosysOceanGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ElosysOceanGUI::message(const QString &title, const QString &message, unsigned int style, int *ret)
{
    QString strTitle = tr("Elosys"); // default title
...
void ElosysOceanGUI::changeEvent(QEvent *e)
{
...
void ElosysOceanGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ElosysOceanGUI::showEvent(QShowEvent *event)
{
    (void)event;
...
#ifdef ENABLE_WALLET
void ElosysOceanGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
...
void ElosysOceanGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ElosysOceanGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ElosysOceanGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
/*
bool ElosysOceanGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
*/
void ElosysOceanGUI::setHDStatus(int hdEnabled)
{
...
void ElosysOceanGUI::setEncryptionStatus(int status)
{
    //Check Settings
    QSettings settings;
    bool fEnableZSigning = settings.value("fEnableZSigning").toBool();
...
void ElosysOceanGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ElosysOceanGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ElosysOceanGUI::detectShutdown()
{
...
void ElosysOceanGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ElosysOceanGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

void ElosysOceanGUI::ShowProgress(QString title, int nProgress)
{
    if (nProgress == 0)
    {
...
// Default: BTN_CANCEL
static int ThreadSafeMessageBox(ElosysOceanGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ElosysOceanGUI::subscribeToCoreSignals()
{
...
void ElosysOceanGUI::unsubscribeFromCoreSignals()
{
...
void ElosysOceanGUI::toggleNetworkActive()
{
