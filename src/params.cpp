...
void initalizeMapParamBootstrap() {
  ...
  bootFile.URL = "http://bootstrap.arrr.black/ELO-bootstrap-v2.tar.gz";
  bootFile.verified = false;
  bootFile.path = GetDataDir() / "ELO-bootstrap.tar.gz";
  bootFile.dlnow = 0;
  bootFile.dltotal = 0;
...
void getHttpsJson(std::string url, JsonDownload *reply, int headerType)
{
    ...
    switch(headerType) {
      ...
      case GITHUB_HEADERS:
          ...
          headers = curl_slist_append(headers, "User-Agent: Elosys");
          break;
    }
