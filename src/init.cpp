...
//     }
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Elosys is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Elosys is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    //Parameter set on initial creation of ELOSYS.conf.
    ...
    if (newInstall && !IsArgSet("maxconnections")) {
        ...
        if (fColdStorage_Offline==CClientUIInterface::BTN_NO) {
            ...
            //Note: By this time the GUI configuration files: '~/.config/Elosys Chain/Treasure\ Chest.conf' and
            //      ELOSYS.conf are already created. We'll have to update the GUI configuration from
            //      the UI code
        }
    }
    ...
    {
        //Note: 'bootstrapinstall' also set when ELOSYS.conf is created
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Elosys") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Elosys to complete") << "\n";
