...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
    ...
    {
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Elosys is not connected!");
    }

    // currently we have checkpoints only in KMD chain, so we checking IsInitialBlockDownload only for KMD itself
    if (chainName.isSymbol("ELOSYS") && IsInitialBlockDownload()) {
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Elosys is downloading blocks...");
    }
