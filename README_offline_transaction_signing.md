...
## Introduction
The problem with traditional wallets and how elosyschain.com addresses these
shortfalls:
...
5. Offline cold storage
   ...
   *  How do you spend the funds in an offline address?
   ...
   Elosys's Treasure Chest wallet overcomes these obstacles as follow:

   Viewing the transaction history
   -------------------------------
   The Elosys blockchain is encrypted. None of the transaction data is visible on
   ...
   To overcome the limitation imposed by the encrypted blockchain, Elosys splits 
...
6. Communicatin
   ...
   be able to figure out that you're transacting in Elosys coin with your neighbour or
   somebody on the otherside of the globe. In an age of unpresidented survailence Elosys
...
7. Summary
   Elosys's Treasure Chest wallet gives you access to these features today:
   * BIP39 master seed 
...
2. You can obtain the software from the official Elosys website, elosyschain.com:
   https://elosyschain.com/wallets/treasure-chest or a source copy from GitHUB at
   https://github.com/ElosysNetwork/elosys 
...
4.1 Installation
   ...
   # dpkg -i elosys-qt-ubuntu1804-v5.7.5.deb
   The application binary is installed in /usr/local/bin/elosys-qt
  ...
  the data is stored:
      ...
      ~/.komodo/ELOSYS -- Contains your wallet and the blockchain
...
4.2 First run
    From a terminal console, launch the application: elosys-qt
    ...
    With no files present in the ~/.komodo/ELOSYS directory the application will ask
    ...
    2. A popup appears with the title: New installation is detected.
       ...
       We'll use the ELO-bootstrap.tar.gz in the following steps.
...
4.3 Blockchain bootstrap
    On a terminal console, navigate to $HOME/.komodo/ELOSYS.
         $ cd ~
         $ cd .komodo/ELOSYS
    ...
    directory:
         $ tar -xvzf ~/Download/ELO-bootstrap.tar.gz
...
4.4 Second run
    From a terminal console, run the application: elosys-qt
...
5.2 First run
    ...
    With no config files in the ~/.komodo/ELOSYS directory the application will ask
    a couple of setup questions:
    1. A popup appears with the title: New installation is detected.
       The dialog asks if you want to run in online mode or offline mode.
...
7. Changes from the previous Treasure Chest version
   Previously you had to manually edit the ELOSYS.conf file in the
...
2. Export an address to the online wallet
   ...
   elosys coins to you. You use the extended viewing key to monitor the balance 
...
## Transactions   
                                       ATTENTION
   For this demonstration, please use very small amounts, like 2 or 3 milliElo
...
1. Fund the new address
   ...
   some elosys coins or you'll need to purchase some Elosys coin on an exchange,
...
2.3 Send the signed transaction (online wallet)
   ...
   The transaction will be submitted to the elosys network for processing.
...
3.1 Construct the transaction (online wallet)
   ...
   - FEE    - How many coins you want to pay the elosys network backoffice in fees
              for maintaining the elosys network. In the transaction text is is 
...
3.3 Send the signed transaction (online wallet)
   ...
   If the transaction is accepted it will be submitted to the elosys network for 
   processing.
